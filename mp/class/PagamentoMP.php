...
$conn = iniciaConexao();
 ...
 class PagamentoMP{
    ...
    $preference_data = array(   
        ...
        "back_urls" => array(
            "success" => $url."MercadoPagoAPI/notifica.php?success",
            "failure" => $url."MercadoPagoAPI/notifica.php?failure",
            "pending" => $url."MercadoPagoAPI/notifica.php?pending"
        ),

        "notification_url" => $url."MercadoPagoAPI/notifica.php",
      ...
      // Criar link para o botão de pagamento normal ou sandbox
        ...
        $this->btn_mp .= 'name="MP-Checkout" >Continuar</a>';
   ...
   public function Retorno($id){
     ...
     $ref = $payment_info["response"]["collection"]["external_reference"];
      ...
      $query = atualizaFatura($status, $forma, $ref);
      if($query){
         return true;
      }
