...
function atribuiValoresDaSessao($idCliente) {
	...
	if (!$conexao) {
		...
		$_SESSION["cpf_cliente"] = $colunaSQL["cpf_cliente"];
...
function atribuiValoresDaSessaoClientesAtivos($idCliente) {
	...
	if (!$conexao) {
		...
		$comandoSQL->execute();
		$colunaSQL = $comandoSQL->get_result()->fetch_assoc();
		$_SESSION["id_cliente"] = $colunaSQL["id_cliente"];
		$_SESSION["id_plano"] = $colunaSQL["id_plano"];
		$_SESSION["nome_cliente"] = $colunaSQL["nome_cliente"];
		$_SESSION["nomePlano"] = $colunaSQL["nome_plano"];
		$_SESSION["valor"] = $colunaSQL["valor"];
		$_SESSION["data_adesao"] = $colunaSQL["data_adesao"];
...
function qtdeDeVendasMes()
{
	$conexao = iniciaConexao();
	$sql = "SELECT * FROM clientes_ativos WHERE 
    MONTH(data_adesao) = MONTH(NOW())";
	...
	$_SESSION['qtdeVendasMes'] = $result;
}

function qtdeDeUserMes()
{
	$conexao = iniciaConexao();
	$sql = "select * from clientes where MONTH(data_criacao) = MONTH(NOW())";
	...
	$_SESSION['qtdeUserMes'] = $result;
}

function qtdeDeUserAtivos()
{
	$conexao = iniciaConexao();
	$sql = "select * from clientes_ativos";
	...
	$_SESSION['qtdeUserAtivos'] = $result;
}
...
function tentaIniciarUmaSessaoPainel($email, $senha) {
